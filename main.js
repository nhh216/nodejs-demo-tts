...
router.get('/test-tts', function(req, res, next) {
  ...
  console.log(text)
  fs.writeFile("/root/tts-demo/clone-tts/assets/transcript.txt", `${text}`, function(err) {
    if(err) {
      return console.log(err);
    }
  });
  const fileName = `audio_${Date.now()}.wav`;
  const FILE_NAME = `assets/infore/${fileName}`
  exec(`FILE_NAME='${FILE_NAME}' /root/tts-demo/clone-tts/scripts/quick_start.sh`,
    (error, stdout, stderr) => {
      console.log(stdout);
      console.log(stderr);
      if (error !== null) {
